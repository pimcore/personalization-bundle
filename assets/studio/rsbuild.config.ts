...
export default defineConfig({
  ...
  dev: {
    ...(!isDevServer ? {assetPrefix: '/bundles/pimcorepersonalization/studio/build/' + buildId} : {}),
    client: {
      host: 'localhost',
      port: 3032,
      protocol: 'ws'
  ...
  output: {
    manifest: true,
    assetPrefix: '/bundles/pimcorepersonalization/studio/build/' + buildId,
    distPath: {
      root: buildPath
    },
  },
  tools: {
    bundlerChain: (chain, { env }) => {
      chain.output.uniqueName('pimcore_personalization_bundle');
    },
  },
  plugins: [
    ...
    pluginModuleFederation({
      name: 'pimcore_personalization_bundle',
